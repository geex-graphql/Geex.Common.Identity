...
namespace Geex.Common.Identity.Core
{
    public class UserHandler : IRequestHandler<AssignRoleRequest, Unit>,
        IRequestHandler<UploadProfileRequest, Unit>,
        ...
        IRequestHandler<QueryInput<IUser>, IQueryable<IUser>>
    {
        ...
        /// <param name="cancellationToken">Cancellation token</param>
        /// <returns>Response from the request</returns>
        public async Task<Unit> Handle(UploadProfileRequest request, CancellationToken cancellationToken)
        {
            var user = await DbContext.Find<User>().OneAsync(request.UserId.ToString(), cancellationToken);
            if (!request.NewAvatar.IsNullOrEmpty())
            {
                var userAvatarClaim = user.Claims.GetOrAdd(x => x.ClaimType == GeexClaimType.Avatar, () => new UserClaim(GeexClaimType.Avatar, request.NewAvatar));
            }
